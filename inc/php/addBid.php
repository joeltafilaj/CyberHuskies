        ...
        if (mysqli_num_rows($resultVerified) == 1) {
            while ($rowVerified = mysqli_fetch_assoc($resultVerified)) {
                ...
                if ($rowVerified['verified'] != 0) {
                    ...
                    $resultCheckBid = mysqli_query($connection, $sqlCheckBid);
                    ...
                    if (mysqli_num_rows($resultCheckBid) == 0) {
                        ...
                        $sqlInsertBid = "INSERT INTO bid VALUES(".$_SESSION['costumer_id'].", $product_id, $bid, '$sessionid', 0, NULL)";
                        ...
                        while ($rowCheckBid = mysqli_fetch_assoc($resultCheckBid)) {
                            if ($bid > $rowCheckBid['offer']) {
                                // Updating bid
                                $sqlInsertBid = "UPDATE bid SET offer = $bid WHERE costumer_id = ".$_SESSION['costumer_id']." AND product_id = $product_id";
                                if (mysqli_query($connection, $sqlInsertBid)) {
                                    // Checking if new bid is the highiest bid or not
                                    $sqlCheckHighiest = "SELECT bid_now, name FROM product WHERE product_id = $product_id";
                                    $resultCheckHighiest = mysqli_query($connection, $sqlCheckHighiest);
